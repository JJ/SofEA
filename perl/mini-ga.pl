...
do {
  ...
  if ( !@{$view->{'rows'}}  )  {
    ...
    my @top_guys = 
      nkeytop { $fitness_of{ $_} } -scalar( keys %fitness_of )/2 
	=> keys %fitness_of;
    my @bottom_guys =  nkeytop { $fitness_of{ $_} } scalar( keys %fitness_of )/2 
	=> keys %fitness_of;
    my @new_docs;
    # Add new of the top half
    for my $t (@top_guys ) {
      if ( $new_guys{$t} ) {
    ...
    # Eliminate old of the bottom half
    for my $b (@bottom_guys ) {
      if ( $old_guys{$b} ) {
	my 	$new_guy= $db->newDoc($b,  
				      $old_guys{$b}->{'_rev'}, 
				      { str => $b,
